...
class GetActivityBloc{
  ...
  PublishSubject<List<ActivitySingleResponse>> _stateStreamController = new PublishSubject();
  StreamSink<List<ActivitySingleResponse>> get _getActivitySink => _stateStreamController.sink;
  Stream<List<ActivitySingleResponse>> get getActivtyStream => _stateStreamController.stream;

  //user input event StreamController
  PublishSubject<GetActivityAction> _eventStreamController = new PublishSubject();
    ...
    _eventStream.listen((event) async {
      if (event == GetActivityAction.FetchActivity){
        for (var activity in this.favoriteActivityList) {
          ActivitySingleResponse activitySingleResponse = await ActivityService.getActivity(activity);
