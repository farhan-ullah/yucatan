...
class TransactionsForDateRangeBloc{
  ...
  PublishSubject<TransactionMultiResponseEntity> _stateStreamController = new PublishSubject();
  StreamSink<TransactionMultiResponseEntity> get _vendorTransactionSink => _stateStreamController.sink;
  Stream<TransactionMultiResponseEntity> get vendorTransactionStream => _stateStreamController.stream;

  //user input event StreamController
  PublishSubject<TransactionsForDateRangeBlocAction> _eventStreamController = new PublishSubject();
  StreamSink<TransactionsForDateRangeBlocAction> get eventSink => _eventStreamController.sink;
  Stream<TransactionsForDateRangeBlocAction> get _eventStream => _eventStreamController.stream;

  TransactionsForDateRangeBloc() {
    _eventStream.listen((event) async {
      if (event == TransactionsForDateRangeBlocAction.FetchVendorTransactions){
        TransactionMultiResponseEntity response = await BookingService.getTransactionsForDateRange();
