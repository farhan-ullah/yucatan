...
class VendorPayoutsBloc{
  ...
  PublishSubject<VendorPayoutsResponse> _stateStreamController = new PublishSubject();
  StreamSink<VendorPayoutsResponse> get _vendorPayoutSink => _stateStreamController.sink;
  Stream<VendorPayoutsResponse> get vendorPayoutStream => _stateStreamController.stream;

  //user input event StreamController
  PublishSubject<VendorPayoutAction> _eventStreamController = new PublishSubject();
  ...
  VendorPayoutsBloc() {
    _eventStream.listen((event) async {
      if (event == VendorPayoutAction.FetchPayoutData){
        VendorPayoutsResponse response = await VendorPayouts.getPayoutsForVendor();
